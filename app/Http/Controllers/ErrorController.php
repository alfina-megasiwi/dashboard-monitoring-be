        ...
        for ($key = 0; $key < count($array_keys); $key++) {
            ...
            for ($error = 0; $error < count($date_error_occur); $error++) {
                if (in_array($date_error_occur[$error], $dates)) {
                    array_push($arr[$array_keys[$key]], [$date_error_occur[$error], (((($data[$array_keys[$key]])["DATE"])[$date_error_occur[$error]])["SOLVE"])]);
                }
            }
            $arr_size = count($arr[$array_keys[$key]]);
            if ($arr_size < 7) {
                for ($i = 0; $i < 7 - $arr_size; $i++) {
                    array_push($arr[$array_keys[$key]], "");
                }
            }
        }
